...
Methods: PCGs are resampled to 1000 Hz, then filtered with a Butterworth band-pass filter of order 3 and cutoff frequencies 25 - 400 Hz and z-score normalized. Deep neural networks (DNNs) which accept waveform inputs are used to make 3-class murmur classification, namely ``Present'', ``Absent'' and ``Unknown'', for each recording. Additional heads for outcome prediction and segmentation are added into the networks to enable multi-objective learning. We performed neural architecture searching (NAS) among a set of network backbones, including multi-branch convolutional neural networks (CNNs), SE-ResNets, TResNets, simplified wav2vec2, etc.
